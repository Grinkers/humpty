...
impl HumptyServer {
    pub(crate) fn new(default_subapp: SubApp, sub_apps: Vec<SubApp>, error_handler: ErrorHandler, not_found_handler: NotFoundHandler, timeout: Option<Duration>) -> Self {
        HumptyServer {
            default_subapp,
            error_handler,
            not_found_handler,
    ...
    pub fn handle_connection<T: IntoConnectionStream>(&self, stream: T) -> io::Result<()> {
        ...
        loop {
            ...
            if request.version == HttpVersion::Http11 && request.headers.get(&HeaderType::Upgrade) == Some("websocket") {
            ...
            let keep_alive = request.version == HttpVersion::Http11 &&
                request.headers.get(&HeaderType::Connection)
            ...
            }.or_else(|e| (self.error_handler)(&request, e)).unwrap_or_else(|e| self.fallback_error_handler(&request, e));
            ...
            if request.version == HttpVersion::Http11 {
                ...
                if !previous_headers.is_empty() {
                    ...
                    return Err(io::Error::new(io::ErrorKind::InvalidInput, "Endpoint has set banned header 'Connection'"));
                }
            }
    ...
    fn call_websocket_handler(&self, request: &Request, stream: &dyn ConnectionStream) {
        ...
        if let Some(handler) =
            self.default_subapp.websocket_routes.iter().find(|route| route.route.route_matches(&request.path))
        {
            handler.handler.serve(request.clone(), stream.new_ref())
        }
    }
    ...
    fn fallback_error_handler(&self, request: &Request, error: io::Error) -> Response {
        error!("Error handler failed. Will respond with empty Internal Server Error {} {} {:?}", request.method, request.path.as_str(), error);
